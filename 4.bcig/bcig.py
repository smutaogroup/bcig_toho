...
def compute_bcig(sysname, pathname, fg, ipath, nrep=50):
    '''Compute the path-wise numerical gradients of npaths.'''
    ...
    cummulative_integrads = numpy.sum(numpy.abs(numpy.cumsum(fg_gradients, axis=1)), axis=1) \
                          + numpy.abs(numpy.sum(fg_gradients, axis=1))  # sum through path to include the contribution of acyl-enzyme (last replica)

    barriers = iomisc.load_pred_barriers(sysname, pathname)[ipath]

    # toho/cex:r2ae split measurement 
    ds_idx = numpy.where(barriers < 32) 
    barriers = barriers[ds_idx]
    cummulative_integrads = cummulative_integrads[ds_idx]

    # gaussian or mixture gaussian
    # gauss_prob = gaussian_prob(barriers)
    gauss_prob = 1 # gaussian_mixture_prob(barriers)
    bolzm_prob = 1 # boltzmann_prob(barriers)

    bcig = numpy.sum(cummulative_integrads * gauss_prob * bolzm_prob)
    return bcig

def fg_bcig(sysname, pathname, npath):
    ...
    ipath = numpy.linspace(0, 199, 200, dtype=int) if sysname != 'both_cex' else numpy.linspace(200, 399, 200, dtype=int)
    
    _sysname = 'both' if sysname[:4] == 'both' else sysname

    fg_bcig = []
    for fg in fgroups:
        bcig_ipath = compute_bcig(_sysname, pathname, fg, ipath) # * npath # scale by the number of pathways so that the magnitude are compatible.
        # verbose.
        print(f'Done: {sysname:>10} {pathname} {fg} npath_{npath} {bcig_ipath}', flush=True)

        fg_bcig.append(bcig_ipath)

    # numpy.save(f'./gauss_bcigs_split/{sysname}.{pathname}.npath_{npath}.npy', numpy.asarray(fg_bcig))

if __name__ == '__main__':
    fg_bcig('toho_cex', 'r2ae', 104)
    exit()
    
    sysnames  = ['toho_amp', 'toho_cex', 'toho_amp', 'toho_cex', 'both_amp', 'both_amp', 'both_cex', 'both_cex']
    pathnames = ['r1ae',     'r1ae',     'r2ae',     'r2ae',     'r1ae',     'r2ae',     'r1ae',     'r2ae']

    for ds_idx in range(8):
        sysname   = sysnames[ds_idx]
        pathname  = pathnames[ds_idx]
        npath=200
        p = multiprocessing.Process(target=fg_bcig, args=(sysname, pathname, npath))
        p.start()